        ...
        return (
            <div>
                {this.state.currentPage && this.state.users[this.state.currentPage].map((ele, index) => { //callback function arrow
                    return <div style={{ display: 'flex', alignItems: 'center', background: 'white', padding: '10px', border: '1px solid #ccc' }}>
                        <img style={{ width: '75px', height: '75px', borderRadius: '50%' }} src={ele.avatar} />
                ...
                <div style={{ marginTop: '10px', display: 'flex', alignItems: 'center' }}>
                    {
                        this.state.totalPage.map((ele, index) => {
                            return <div onClick={() => this.fetchingUsers(ele)} style={{ width: '20px', marginRight: '10px', cursor: 'pointer', height: '20px', background: 'white' }}>{ele}</div>
                        })
                    }
                </div>
            </div>
